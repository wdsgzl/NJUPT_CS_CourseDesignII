...
<script>
    ...
    function updateMenu(dishid,newprice,newstock){
        ...
        fetch('update_menu.php', {
            ...
            .then(response => response.json())  // 处理返回的 JSON 响应
        .then(data => {
            // 在这里处理返回的结果
            if (data.success) {
                alert("积分更新成功!");
            } else {
                alert("积分更新失败: " + data.error);
            }
        })
    } 
        //}
...
<body>
    <header class="header">
        ...
        <button class="button" onclick='location.href=("show.php")'>积分查询</button>
            <button class="button" id="toggleAdmin">管理员模式</button>
        </div>
    </header>
