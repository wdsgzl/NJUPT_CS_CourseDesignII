...
$dbp_file = '../database/Personal.sqlite3';
$user= $_GET['user'];
$cus=$user;
...
$query='select point from Manager where user = :user';
$stmt = $dbp->prepare($query);
$stmt->bindValue(':user', $user, SQLITE3_TEXT);
$result = $stmt->execute();
$score=$result->fetchArray();
//echo "<script>alert('$score[0]')</script>";
...
<script>
                ...
                <div class="dish-card ${remainingStock === 0 ? 'sold-out' : ''}" data-dish-id="${dish.id}">
                    <div class="dish-info">
                        ...
                        <div class="price-info">
                            <p>积分兑换: ${dish.pointsPrice}积分</p>
                            <p>剩余库存: ${remainingStock}</p>
                        </div>
                        <div class="quantity-controls">
                            ${remainingStock > 0 ? `
                                <button class="button"  onclick="scorechange(${dish.id})">
                                    积分兑换
                                </button>
                            ` : '<span>已售罄</span>'}
                        </div>
    ...
    function scorechange(dishId){
        var nscore=<?php echo intval($score[0]);?>;
        var nuser=<?php echo json_encode($user);?>;
        let result = confirm(`您确定要用${dishes[dishId].pointsPrice}积分兑换一份${dishes[dishId].name}吗？`);
        if(result){ 
            if(nscore<dishes[dishId].pointsPrice){
                alert("抱歉，您的积分不足！");
                location.reload();
            }
            //alert(nscore-dishes[dishId].pointsPrice);
            else{
            updateScore(nuser,dishes[dishId].pointsPrice);
            updateMenu(dishId,dishes[dishId].regularPrice,dishes[dishId].stock-1);
            alert(`您还有${nscore-dishes[dishId].pointsPrice}积分!`);
            //renderDishes();
        }   
        }
        location.reload();
    ...
    function renderCart() {
        ...
        const total = cart.reduce((sum, item) => {
            const dish = dishes.find(d => d.id === item.id);
            const price = isMember ? dish.regularPrice*0.9 : dish.regularPrice;
    ...
    function showpoint(){
        var pscore=<?php echo intval($score[0]);?>;
        alert(`您有${pscore}积分!`)
    }
...
<body>
    <header class="header">
        ...
        <div class="user-controls">
            ...
            <button class="button" onclick="showpoint();">查询积分</button>
        </div>
    </header>
